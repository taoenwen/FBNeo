...
INT32 ChdReadSector(ChdImage* pImage, INT32 nLba, INT32 nDataType, UINT8* pDest)
{
	...
	if (nChdFrame == -2) {
		INT32 nSize = (nDataType == CHD_TRACK_RAW_DONTCARE || nDataType == CHD_TRACK_MODE1_RAW || nDataType == CHD_TRACK_MODE2_RAW) ? 2352 : ChdTypeSize(nDataType);
		if (nSize == 0) {
			return 1;
		}
